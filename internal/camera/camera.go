...
func (c *Camera) WorldToScreen(wx, wy float64) (sx, sy float64) {
	c.ComputeViewMatrix()
	sx, sy = node.TransformPoint(c.viewMatrix, wx, wy)
	return
}
...
func (c *Camera) ScreenToWorld(sx, sy float64) (wx, wy float64) {
	c.ComputeViewMatrix()
	wx, wy = node.TransformPoint(c.invViewMatrix, sx, sy)
	return
}
...
func (c *Camera) VisibleBounds() types.Rect {
	...
	x0, y0 := node.TransformPoint(inv, vx, vy)
	x1, y1 := node.TransformPoint(inv, vr, vy)
	x2, y2 := node.TransformPoint(inv, vr, vb)
	x3, y3 := node.TransformPoint(inv, vx, vb)
